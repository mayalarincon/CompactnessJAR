...
(* This theory provides a background on graph notions and 
   properties.  We follow the approach by L. Noschinski
   available in the AFPs.  But since not all elements are
   required, and also some specialized notions on directed
   bipartite digraphs are required, we prefer not to import
   Noschinski graph theory. 
*)

(* Proof are desiccated in several steps since since the 
focus is clarity instead proof automation *)
